...
Gear is divided among the team as works best but there are usually a few common classes of jobs.
...
- 50ml 'falcon' tubes for making personal solutions for washing forceps etc
- Pasteur-type pipets 
...
![Basic Processing Flow -- Grey boxes are processing stations. White rounded boxes are inputs/outputs of processing stations. Arrows show the flow of processing materials. BOLD arrows show the flow of fly materials. The green box represents materials set aside for processing after the live flies have been processed. \label{fig:flow}](figures/fly_processing_flow.png)
...
1. Write the number that corresponds to this fly in the field record sheet on the slide label.
...
If this fails for _even __ONE___ fly, it will likely cause __ALL SUBSEQUENT fly information__ to be incorrect^[It shifts the register of identities out of sync with all remaining flies.].
...
- The information is recorded in pencil on a pad of small labels that will be placed on each tube (at the __Packaging__ station) that contains part of the fly with the corresponding number (Figure \ref{fig:labels}).
...
1. __Wash tools (forceps/scissors/etc) in 90% EtOH and wipe with Kim-Wipes to avoid contamination of current fly.__
1. Assemble enough tubes (_pre-filled with ~250ul of 90% EtOH or appropriate storage solution_) to receive the number of separate tissues dictated by this collection's needs.
...
1. __Double-check__ that each tube has the correct side-label, then label the tube top with the Site-ID code and fly number only using lab marker/sharpie.
1. If the fly has a positive status, add a note to the side-label.
	- `'+'` = low to normal infection level
	- `'++'` = mid to high infection
	- `'+++'` = very high infection

After the live flies have been processed:
...
1. Get a strip of parafilm with deminsions ~2 inches long/~0.5 inches wide (~5cm Long /~1.5cm Wide).
